const { createApp, ref, getCurrentInstance, reactive, toRefs, computed, watch, watchEffect, provide, inject, onBeforeMount, onMounted, onBeforeUpdate, onUpdated, onBeforeUnmount, onUnmounted } = Vue
...
createApp({
    setup() {
        ...
        const stop = watchEffect(() => text.value = val1.value + " " + val2.value)

        function stopWatch() {
            stop()
        }
        return {
            val1, val2, text, stopWatch
        }
    }
